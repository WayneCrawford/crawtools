...
class Decimator:
    ...
    def update_inventory(self, inv, st):
        """
        Update inventory for channels found in stream

        :param inv: obspy inventory
        :kind inv: obspy.core.events.inventory.Inventory
        :param st: date stream (used to determine net/sta/chan/loc codes)
        :kind inv: obspy.core.stream.Stream
        ...
        for tr in st:
            s = tr.stats
            new_cha = _duplicate_channel(inv, network=s.network,
                                         station=s.station,
                                         location=s.location,
                                         channel=s.channel)
    ...
    def update_inventory_from_nslc(self, inv, network='*', station='*',
        ...
        :param inv: obspy inventory
        :kind inv: obspy.core.events.inventory.Inventory
        :param network: FDSN network code
        :param station: station code
        :param channel: channel code
        :param location: FDSN location code
        ...
        for net in inv_selected:
            for sta in net:
                old_sta = sta.copy()
                for cha in old_sta:
                    new_cha = _duplicate_channel(inv,
                                                 net.code,
                                                 sta.code,
                                                 cha.location_code,
                                                 cha.code)
...
def _duplicate_channel(inv, network, station, location, channel):
    """
