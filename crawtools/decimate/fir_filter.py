...
class FIRFilter:
    ...
    @classmethod
    def from_SAC(cls, decim):
        """
        Make SAC FIR filter

        :param decim: decimation factor (must be one of 2, 3, 4, 5, 6, 7)
        :type decim: int
        """
        if decim not in (2, 3, 4, 5, 6, 7):
            raise ValueError(f'No SAC decimation filter for {decim=}')
        SAC_path = Path(inspect.getfile(
            inspect.currentframe())).resolve().parent / 'sac_fir'
        with open(str(SAC_path / f'dec{decim}')) as f:
            f.readline()
            A = f.readline().split()
            divisor, n_values = float(A[0]),  int(A[1])
            coeffs = [float(x)/divisor for line in f.readlines()
                      for x in line.split()]
        if not len(coeffs) == n_values:
            raise RuntimeError('# FIR values is different from that stated: '
                               f'{len(coeffs)} != {n_values}')
        # SAC only saves right half of symmetrical odd filter
        offset = len(coeffs)-1
        coeffs = coeffs[-1:0:-1] + coeffs
        # Normalize coefficients so that sum = 1 (why aren't the SAC Filter
        # coeffs already normalized?)
        # print(f'{np.sum(coeffs)=:.3f}')
    ...
    def convolve(self, data):
        ...
        cut_data = con_data[self.offset:-(len(self.coeffs)-self.offset-1)]
    ...
    def plot(self, show=False, savefig=None):
        ...
        fig.subplots_adjust(hspace=0.4, wspace=0.2, left=0.07, right=0.99,
                            bottom=0.1, top=0.95)
        if show:
            plt.show()
...
def plot_freqz(ax, b, a=1, desired_nyquist=None):
    ...
    ax.hlines([0, -48.16, -96.33, -144.5, -192.7],
              0., 1.0, lw=0.5, linestyle="--")
...
def plot_phasez(ax, b, a=1, desired_nyquist=None, delay=0):
    ...
    if desired_nyquist:
        ...
        ax.vlines(desired_nyquist, -200, 200, lw=0.5, linestyle="--")
        ax.set_ylim(*ylim)
    ax.hlines(0, 1.0, 0., lw=0.5, linestyle="--")

    ax.set_ylabel('Phase (radians)')
    ax.set_xlabel(r'Normalized Frequency (x$\pi$rad/sample)')


def plot_impz(ax, b, a=1, delay=0.):
    # FIR filter
    if isinstance(a, int):
        lb = len(b)
    # IIR filter.
    ...
    ax.vlines(delay+.5, -2, 2, lw=0.5, linestyle="--")
    ax.set_ylim(*ylim)
    # ax.set_ylim([-.1,1.1])
...
def plot_stepz(ax, b, a=1, delay=0):
    ...
    # ylim = ax.get_ylim()
    ax.vlines(delay+.5, -2, 2, lw=0.5, linestyle="--")
    ax.set_ylim([min(min(step), -.15), max(max(step), 1.15)])
