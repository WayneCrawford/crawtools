...
from .utils import coherence_significance_level
...
class TransferFunctions(object):
    ...
    def plot_one(self, in_chan, out_chan, fig=None, fig_grid=(1, 1),
        ...
        xf[xf == 0] = None
        # VERY SLOW
        # ax_a.vlines(f, np.abs(xf-xferr), np.abs(xf+xferr), label=label)
        # ax_a.set_xscale('log')
        # ax_a.set_yscale('log')
        # FASTER
        ax_a.loglog(f, np.abs(xf+xferr), color='blue', linewidth=0.5)
        ax_a.loglog(f, np.abs(xf-xferr), color='blue', linewidth=0.5)
        ax_a.loglog(f, np.abs(xf), color='black')
