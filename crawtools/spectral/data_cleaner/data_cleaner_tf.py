...
class DataCleaner():
    ...
    def __init__(self, stream, remove_list, noise_channel="output",
                 ...
                 show_plots=False, show_tfs=False, fast_calc=False):
        """
        Args:
            ...
            show_plots (bool): Show progress plots
            show_tfs (bool): Show calculated transfer functions
        ...
        sdfs = [SpectralDensity.from_stream(stream)]
        self.DCTFs = DCTFs()
        ...
        for in_chan in in_list:
            ...
            print('=' * 78)
            print(f'{in_chan=}, {remove_new=}, {remove_seq=}')
            print(f'{ic=}, {out_chans=}')
            print(sdfs[-1])
            ...
            remove_seq += remove_new
            self.DCTFs.append(DCTF(ic, remove_seq, tf))
            if fast_calc:
                sdfs.append(self._removeTF_SDF(
                    sdfs[-1], tf, show_plots, add_suffix=remove_new))
            else:
    ...
    def clean_stream(self, stream, in_time_domain=True, stuff_locations=True):
        """
    ...
    def _verify_untransformed(tfs, out_chan):
        ...
        if not tfs.input_units.lower() == 'counts':
            raise ValueError(f"input units are not counts: {tfs.input_units}")
    ...
    def _removeTF_SDF(self, sdf, tf, show_plots=False, add_suffix=None):
        """
        ...
        Args:
            ...
            tf (:class:`.TransferFunctions`): Transfer Functions w.r.t. one
                input channel
            show_plots (bool): show plots for each step
