    ...
    def ft_subtract_tfs(self, fts):
        ...
        for dctf in self:
            ...
            for out_chan in tfs.output_channels:
                ...
                fts[new_out_chan] = fts[out_chan] - fts[in_chan] * np.conj(tfs.values(out_chan))
        return fts
...
#     @staticmethod
#     def update_channel_names(channels, dctfs):
#         """
#         Update channel names to whatever the last removal makes them
#
#         Args:
#             channels (list): list of channel names
#             dctfs (list of :class:`DCTF`): data cleaner transfer functions
#                 applied
#         """
#         remove_seqs = {}
#         channels = channels.copy()
#         for dctf in dctfs:
#             for oc in dctf.tfs.output_channels:
#                 oc_base = oc.split('-')[0]
#                 remove_seqs[oc_base] = dctf.remove_sequence
#         for k, v in remove_seqs.items():
#             channels[channels.index(k)] = k+v
#         return channels
