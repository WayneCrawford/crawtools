...
class SpectralDensity:
    """
    Matrix of one_sided spectral density functions.
    """
    ...
    def from_stream(cls, stream, window_s=1000, windowtype='prol1pi',
        ...
        if not isinstance(stream, Stream):
            raise ValueError(f'stream is a {type(stream)}, not an obspy Stream')
    ...
    def plot_one_spectra(self, key, subkey, fig=None, fig_grid=(1, 1),
                         ...
                         show_legend=True, title=None, show_coherence=False,
        ...
        if plot_peterson is True and PSD_units.lower() == '(m/s^2)^2':
            lownoise, highnoise = PetersonNoiseModel(f, True)
    ...
    def plot_one_coherence(self, in_chan, out_chan, fig=None, fig_grid=(1, 1),
                           ...
                           show_legend=True, title=None, show_phase=True):
        """
        Plot one coherence
